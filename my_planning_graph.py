...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        return any([~e in self.children[actionA] for e in self.children[actionB]])
    ...
    def _interference(self, actionA, actionB):
        ...
        # TODO: implement this function
        #raise NotImplementedError
        return any([~e in self.children[actionA] for e in self.parents[actionB]]+\
                   [~e in self.children[actionB] for e in self.parents[actionA]])
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        # TODO: implement this function
        #raise NotImplementedError
        return any([self.parent_layer.is_mutex(ea,eb) for ea in self.parents[actionA] for eb in self.parents[actionB]])
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        # TODO: implement this function
        #raise NotImplementedError
        return all([self.parent_layer.is_mutex(ea,eb) for ea in self.parents[literalA] for eb in self.parents[literalB]])
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        level_cnt=0 # Level counter
        level_sum=0 # Summation of level counters at which goals are acheived
        goal_tmp=self.goal.copy()
        goal_len=len(goal_tmp)
        while not self._is_leveled:
            goal_tmp.difference_update(self.literal_layers[-1]) # Remove goals that exist in the last literal layer of the graph plan
            tmp_goal_len=len(goal_tmp)
            if goal_len!=tmp_goal_len: # If yes, then some goals have been achieved in last literal layer
                level_sum+=level_cnt*(goal_len-tmp_goal_len)
                goal_len=tmp_goal_len
            
            if goal_tmp:    # If goal_tmp is not empty, then some, or all goals have not been achieved yet. Thus, extend the graph plan and increment the level counter
                self._extend()
                level_cnt+=1
            else:
                break
    ...
    def h_maxlevel(self):
        ...
        while not self._is_leveled:
            goal_tmp.difference_update(self.literal_layers[-1]) # Remove goals that exist in the last literal layer of the graph plan
            if goal_tmp:    # If goal_tmp is not empty, then some, or all goals have not been acheived yet. Thus, extend the graph plan and increment the level counter
                self._extend()
                level_cnt+=1
            else:
                break
    ...
    def h_setlevel(self):
        ...
        goal_tmp=self.goal.copy()
        while not self._is_leveled:
            if goal_tmp.issubset(self.literal_layers[-1]) and not any([self.literal_layers[-1].is_mutex(g1,g2) for g1 in goal_tmp for g2 in goal_tmp if g1!=g2]):
                break
            else:
                self._extend()
                level_cnt+=1
